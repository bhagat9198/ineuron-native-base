import { Box, Checkbox, Heading, IconButton, Input, Text, useToast, VStack, Center, HStack, Icon, WarningOutlineIcon } from 'native-base';
import React from 'react'
import { Feather, Entypo } from '@expo/vector-icons';

const NBToDolist = (props) => {
  const { label, checkboxReq, list, handler, heading, setList } = props;
      ...
      <VStack space={4}>
        ...
        <VStack space={2}>
          {list.map((item, itemI) => <HStack w="100%" justifyContent="space-between" alignItems="center" key={item.title + itemI.toString()}>
            {checkboxReq && <Checkbox isChecked={item.isCompleted} onChange={() => handleStatusChange(itemI)} value={item.title}></Checkbox>}
